...
class UserViewTestCase(TestCase):
    ...
    def setup_likes(self):
        m1 = Message(text="message one", user_id=self.testuser_id)
        m2 = Message(text="message two", user_id=self.testuser_id)
        m3 = Message(id=4252, text="message three", user_id=self.u1_id)
        db.session.add_all([m1, m2, m3])
        db.session.commit()

        l1 = Likes(user_id=self.testuser_id, message_id=4252)
    ...
    def test_user_show_with_likes(self):
        ...
        with self.client as c:
            ...
            #Test for messages count of 2
            self.assertIn("2", found[0].text)

            #Test for follower count of 0
            self.assertIn("0", found[1].text)

            #Test following count of 0
            self.assertIn("0", found[2].text)

            #Test for like count of 1
            self.assertIn("1", found[3].text)

    def test_add_like(self):
        m = Message(id=2048, text="The earth is round", user_id=self.u1_id)
        db.session.add(m)
        db.session.commit()
        ...
        with self.client as c:
            ...
            resp = c.post("/messages/2048/like", follow_redirects=True)
            self.assertEqual(resp.status_code, 200)

            likes = Likes.query.filter(Likes.message_id==2048).all()
    ...
    def test_remove_like(self):
        ...
        m = Message.query.filter(Message.text=="message three").one()
    ...
    def test_user_show_with_follows(self):
        ...
        with self.client as c:
            ...
            #Test for message count of 0
            self.assertIn("0", found[0].text)

            #Test for following count of 2
            self.assertIn("2", found[1].text)

            #Test for follower count of 1
            self.assertIn("1", found[2].text)

            #Test for like count of 0
