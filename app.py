...
def users_show(user_id):
    ...
    #access user.likes and backref messages model to get liked warble id 
...
def profile():
    ...
    if form.validate_on_submit():
        if User.authenticate(user.username, form.password.data):
            ...
            #instead of writing conditional, python will use image form data if datatype is not None (else uses default image path)
...
def like_warble(message_id):
    """Toggle like on-off for warble for logged-in users."""
...
def display_all_likes(user_id):
    """Display all likes by user."""
